...
function matchesWithResults() {
  ...
  return window.MATCHES.map(function(m) {
    ...
    return Object.assign({}, m, { result: over[key] !== undefined ? over[key] : m.result });
  });
}
function standings() {
  ...
  matchesWithResults().forEach(function(m){
    ...
    var lId = m.result.winner === m.a ? m.b : m.a;
    var l = byId[lId];
    if (!w || !l) return;
    w.mw += 1; l.ml += 1;
...
function render() {
  ...
  if (!nxt) {
    ...
    hero.innerHTML = '<p class="kicker">' + kick + " · Round " + nxt.round + "</p><h1>" + a.name + " vs " + b.name + "</h1><p>Ref " + ref.name + (role ? ' <span class="us-chip">' + role + "</span>" : "") + "</p>";
  }
  var st = standings();
  ...
  document.getElementById("matches").innerHTML = matchesWithResults().map(function(m){
